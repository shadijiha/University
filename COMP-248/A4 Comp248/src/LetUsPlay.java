...
public class LetUsPlay {
	...
	public static void main(String[] args) {
		...
		String userChoice = "0"; // Here using a string instead of int to avoid app crash if user enters a string
									// to scan.nextInt()
		do {
		...
		for (;;) {
			...
			println(String.format("	%s is on level %d at location (%d,%d) and has %d units of energy",
					players[0].getName(), players[0].getLevel(), players[0].getX(), players[0].getY(),
					players[0].getEnergy()));
			println(String.format("	%s is on level %d at location (%d,%d) and has %d units of energy.",
					players[1].getName(), players[1].getLevel(), players[1].getX(), players[1].getY(),
					players[1].getEnergy()));
	...
	public static void movePlayer(Player p, Board b, Dice d, Player opponent, Scanner scanner) {
		...
		if (!hasEnoughEnergy(p)) {
			...
			if (allowMove) {
				...
				println(String.format("	Your energy is adjusted by %d for landing at (%d,%d) at level %d",
						energyOnCurrentPos, temp.getX(), temp.getY(), temp.getLevel()));
	...
	public static int adjustEnergy(Player p, Dice d) {
		...
		if (!hasEnoughEnergy(tempCopy)) {
			...
			println(String.format(
					"%s did not have enough energy to move. He gains a chance to get 2 bonus energy units if he rolls a double. This process is done 3 times for up to %d energy units. %s has gained %d bonus energy units.\n",
