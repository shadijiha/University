...
public class Matrix {
	 ...
	 */
	public static Matrix generateIdentity(int rows, int cols) {
	 ...
	 */
	public Matrix multiply(final Vec3 mat) throws Exception {
		throw new Exception("c'est de l'asti de sauce");
...
//		return result;
	...
	public Matrix multiply(final Vec4 vec4) {

		Matrix m = new Matrix(4, 1);
		m.setData(0, 0, vec4.x);
		m.setData(1, 0, vec4.y);
		m.setData(2, 0, vec4.z);
		m.setData(3, 0, vec4.w);
	 ...
	 */
	public Vec3 toVector3D() throws Error {
		if (this.cols == 1) {
			if (this.rows == 1) {
				return new Vec3(this.getData(0, 0), 0);
			} else if (this.rows == 2) {
				return new Vec3(this.getData(0, 0), this.getData(1, 0));
			} else if (this.rows == 3) {
				return new Vec3(this.getData(0, 0), this.getData(1, 0), this.getData(2, 0));
			} else {
	...
	public Vec4 toVector4D() {
		return new Vec4(getData(0, 0), getData(1, 0), getData(2, 0), getData(3, 0));
	}
